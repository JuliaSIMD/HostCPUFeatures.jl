...
_pick_vector_width_float16(::StaticInt{RS}, ::True) where {RS} =
  StaticInt{RS}() ÷ StaticInt{2}()
_pick_vector_width_float16(::StaticInt{RS}, ::False) where {RS} =
  StaticInt{RS}() ÷ StaticInt{4}()
pick_vector_width(::Type{Float16}) =
  _pick_vector_width_float16(register_size(Float32), fast_half())
pick_vector_width(::Type{T}) where {T} = register_size(T) ÷ static_sizeof(T)
@inline @constprop :aggressive function _pick_vector_width(
  min_W,
  max_W,
  ::Type{T},
  ::Type{S},
...
@inline @constprop :aggressive function pick_vector_width(
  ::Type{T},
  ::Type{S},
  args::Vararg{Any,K},
) where {T,S,K}
